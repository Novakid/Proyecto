...
export class AuthService {
  ...
  async login(dto: LoginDto) {
    const user = await this.users
      ...
      .getOne();
    if (!user?.passwordHash || user.estatus !== 1 || !(await compare(dto.password, user.passwordHash))) {
  ...
  async createUser(dto: CreateAuthUserDto) {
    ...
    const saved = await this.users.save(user);
    return { id: saved.id, nombre: saved.Nombre, email: saved.email, role: saved.role };
  }

  private async issueToken(user: Usuario) {
    return {
      accessToken: await this.jwtService.signAsync({ sub: user.id, email: user.email, role: user.role }),
