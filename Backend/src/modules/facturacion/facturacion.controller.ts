...
export class FacturasController {
  ...
  @Get()
  findAll(
    @Query() filters: FilterFacturasDto,
    @Req() req: AuthenticatedRequest,
  ) {
    return this.service.findAll(
      filters,
      req.user!.sub,
      req.user!.permissions?.includes('facturacion.ver_todas') ?? false,
    );
  }

  @Get('catalogos/vendedores')
  vendedores(@Query('search') search = '') {
    return this.service.buscarVendedores(search);
  }

  @Get('catalogos/clientes')
  clientes(@Query('search') search = '') {
    return this.service.buscarClientes(search);
  }

  @Get('catalogos/productos')
  productos(@Query() query: SearchFacturaCatalogDto) {
    return this.service.buscarProductos(query.search, query.limit);
  }
  ...
  precio(
    ...
    return this.service.precioEfectivo(
      Number(clienteId || 0),
      Number(productoId),
    );
  }
